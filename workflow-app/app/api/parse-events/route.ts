...
export async function POST(req: Request) {
  try {
    ...
    const prompt = `
Parse the following event description into a JSON object with the exact fields:
{
  ...
  "reminder": string,           // required, choose from: "10 minutes before", "1 hour before", "1 day before", "2 days before", or "none"
                                ...
                                // then choose the best matching option. If it's unclear but a reminder feels implied (like for early meetings, deadlines, etc.), choose "1 hour before" by default.
